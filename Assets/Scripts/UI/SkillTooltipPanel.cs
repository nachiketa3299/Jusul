...
namespace Jusul
{
  ...
  public class SkillTooltipPanel : MonoBehaviour
  {
    ...
    [Header("Skill Name")][Space]
    [SerializeField] TMP_Text _skillName;

    [Header("Skill Rarity(0부터 5까지)")][Space]
    [SerializeField] TMP_Text _skillRarityText;
    [SerializeField] Image _skillRarityBackground;
    [SerializeField] List<Color> _colorOnRarity = new();

    [Header("Description")][Space]
    [SerializeField] TMP_Text _description;

    [Header("Attack Infos")][Space]
    ...
    public string GetRarityName(SkillRarity rarity) => rarity switch
    {
      SkillRarity.Normal => "일반",
      SkillRarity.Rare => "희귀",
      SkillRarity.Hero => "영웅",
    ...
    public void SetSkillInfo(SkillBase skill)
    {
      ...
      _skillRarityText.text = GetRarityName(skill.Rarity);
      _skillRarityBackground.color = _colorOnRarity[(int)skill.Rarity];
